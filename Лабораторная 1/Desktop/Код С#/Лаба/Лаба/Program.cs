...
class V2DataOnGrid : V2Data
{
    ...
    public void InitRandom(double minValue, double maxValue)
    {
        ...
        Node_Val = new Complex[2,Elem[0].Node*Elem[1].Node + 1];
        ...
        for (int i = 0; i < Elem[0].Node; i++)
        {
            for (int j = 0; j < Elem[1].Node; j++)
            {
                k++;
                //complex coord
                Node_Val[0, k] = new Complex(i * Elem[0].Step, j * Elem[1].Step);

                var next_Re = rnd.NextDouble();
                var next_Im = rnd.NextDouble();

                //complex value
                Node_Val[1, k] = new Complex(minValue + (next_Re * (maxValue - minValue)),
                                             minValue + (next_Im * (maxValue - minValue)));

                // a + i(b): Node_Val[i,0] = (a,b) 
                //           double minValue << a << double maxValue
                //           double minValue << b << double maxValue
    ...
    public static implicit operator V2DataCollection(V2DataOnGrid a)
    {
        ...
        for(int ind = 1; ind < a.Node_Val.Length/2; ind++)
        { 
        
            DataItem DI = new DataItem();
         
            DI.Vect2 = new Vector2((float)a.Node_Val[0, ind].Real,
                                  (float)a.Node_Val[0, ind].Imaginary);

            DI.Compl = new Complex((float)a.Node_Val[1, ind].Real,
                                  (float)a.Node_Val[1, ind].Imaginary);
            V2.Data.Add(DI);
    ...
    public override Complex[] NearAverage(float eps)
    {
        Complex mid_value = 0;
        int ind = -1;
        for(int k = 0; k < Elem[0].Node * Elem[1].Node + 1; k++)
        {
            mid_value += Node_Val[1, k];
        }
       
        mid_value = mid_value.Real / (Elem[0].Node * Elem[1].Node + 1);
     
        for (int j = 0; j < Elem[0].Node * Elem[1].Node + 1; j++)
        {
            if (Math.Abs(mid_value.Real - Node_Val[1, j].Real) <= (double)eps)
            {
                ind++;
            }
        }
    ...
    public override string ToLongString()
    {
        ...
        for (int i = 1; i < Elem[0].Node*Elem[1].Node + 1; i++)
        {
                long_res = long_res + '\n' + Node_Val[0,i] + " = " + Node_Val[1, i];
        }
